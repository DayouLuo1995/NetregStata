...
{pstd}
{cmd:netreg} provides methods for performing regression of a network response, where each data point represents an edge on a network, or covariates of interest. 
 ...
 With this new assumption, we can provide a better estimation for coefficients¡¯ covariance matrix, resulting in an accurate confidence interval for coefficients.
 Also, applying exchangeable assumption enables the iteratively reweighted least squares method for better estimation.  For more details, see Marrs (2017). This package is also avaliable in R, you can search netregR in R for this package.
 {p_end}
...
{pstd}
The {opt reweight} option indicates that iteratively reweighted least squares should be used to compute estimate of coefficients. 
...
{pstd}Here is an example of network linear regression with exchangeable assumption. Data used here is a wolf data from Junk, (1987). It is now available on author's github website.
